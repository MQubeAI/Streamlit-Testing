...
st.set_page_config(page_title="FenerGPT", page_icon="images/Fener_logo.png",
        ...
        if player_detail_opt == "Passport":
            ...
            with st.chat_message("system"):
                ...
                cols[1].button("yes", use_container_width=True,
                               on_click=show_upload, args=[True])
                cols[2].button("no", use_container_width=True,
                               on_click=show_upload, args=[False])
            ...
            if "chat_engine" not in st.session_state.keys():
                st.session_state.chat_engine = index.as_chat_engine(
                    ...
                    context_prompt=(
                        ...
                        "Instruction: Use the previous chat history, or the context above, to interact and help the user. Whenever the user writes 'generate the word document', 'generate the contract', or similar phrases that indicate they want to generate the player contract but only when user tells about genration of doucment , FenerGPT should respond with 'Yes' in the chat to confirm the action.\n"
            ...
            language = st.sidebar.radio("Select the language for voice input:", options=[
                                        "English", "Turkish"], index=0)
